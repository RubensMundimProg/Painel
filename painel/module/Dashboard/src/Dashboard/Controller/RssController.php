...
class RssController extends AbstractEstruturaController
{
    ...
    protected function downloadInmetFeed($url, array &$errors = [])
    {
        ...
        $response = $this->downloadInmetFeedUsingStream($url, $headers, $errors);
        if ($response !== null) {
            return $response;
        }

        $response = $this->downloadInmetFeedUsingCurl($url, $headers, $errors);
        if ($response !== null) {
            return $response;
        }
    ...
    protected function downloadInmetFeedUsingStream($url, array $headers, array &$errors)
    {
        $options = [
            'http' => [
                'timeout' => 15,
        ...
        $response = $this->attemptStreamDownload($url, $options, $errors, false);
        if ($response !== null) {
            return $response;
        }
        ...
        return $this->attemptStreamDownload($url, $options, $errors, true);
    }

    protected function attemptStreamDownload($url, array $options, array &$errors, $insecure)
    {
        ...
        if ($response !== false) {
            if ($insecure) {
                $errors[] = 'Conexão estabelecida sem validação de certificado SSL (stream).';
            }
        ...
        $message = $lastError && isset($lastError['message']) ? $lastError['message'] : 'erro desconhecido';
        $errors[] = sprintf(
            'Falha ao acessar %s via stream%s: %s',
            $url,
            $insecure ? ' (SSL não verificado)' : '',
            $message
        );
    ...
    protected function downloadInmetFeedUsingCurl($url, array $headers, array &$errors)
    {
        if (!function_exists('curl_init')) {
            $errors[] = 'Extensão cURL não disponível no servidor.';
            return null;
        }
        ...
        $response = $this->attemptCurlDownload($url, $options, $errors, false);
        if ($response !== null) {
            return $response;
        }
        ...
        return $this->attemptCurlDownload($url, $options, $errors, true);
    }

    protected function attemptCurlDownload($url, array $options, array &$errors, $insecure)
    {
        $ch = curl_init($url);
        ...
        if ($result !== false) {
            if ($insecure) {
                $errors[] = 'Conexão cURL estabelecida sem validação de certificado SSL.';
            }
        ...
        $errors[] = sprintf(
            'cURL (%d) falhou ao acessar %s%s: %s',
            $errno,
            $url,
            $insecure ? ' sem verificação SSL' : '',
    ...
    protected function getInmetCachePath()
    {
        ...
        $candidates = [
            $rootDir . '/data/rss/inmet.xml',
            getcwd() . '/data/rss/inmet.xml',
        ];
