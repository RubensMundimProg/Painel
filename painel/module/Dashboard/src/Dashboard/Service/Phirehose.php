...
abstract class Phirehose
{
     ...
     * Member Attribs
     */
    protected $holdTimeForCheckAbort = 15;
    protected $timeLastTweet = 0;
    protected $username;
    protected $password;
    protected $method;
    protected $format;
    ...
    //ABORTA ROTINA SE USUÁRIO ALTEROU O CONJUNTO DE PALAVRAS.
    public function abortRotine()
    {
    ...
    public function consume($reconnect = TRUE)
    {
        ...
        do {
            ...
            while ($this->conn !== NULL && !feof($this->conn) &&
                ...
                //CHECAR SE É PRECISO ABORTAR ROTINA SE NÃO EXISTE TWEET DENTRO DE X TEMPO
                if($this->timeLastTweet == 0) $this->timeLastTweet = time();
                if ((time() - $this->timeLastTweet) >= $this->holdTimeForCheckAbort) {
                    $this->holdTimeForCheckAbort += 15;
                    $this->abortRotine();
                }
