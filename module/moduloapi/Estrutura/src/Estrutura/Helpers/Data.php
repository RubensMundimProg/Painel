...
class Data {
    ...
    public static function get($date, $tipo) {
        ...
        if ($date) {
            ...
            return self::datePart(self::toDateTime($date), $tipo);
        } else {
    ...
    /**
     *
     * @param \DateTimeInterface|string $data
     * @return boolean
     */
    public static function isValid($data) {

        $data = self::toDateTime($data);
        $d = (int) $data->format('d');
        $m = (int) $data->format('m');
        $y = (int) $data->format('Y');
    ...
    public static function calculaData($data) {
        ...
        //defino data 1
        $ano1 = (int) $data->format('Y');
        $mes1 = (int) $data->format('m');
        $dia1 = (int) $data->format('d');
    ...
    private static function toDateTime($date) {
        ...
        if (is_numeric($date)) {
            ...
            return $dateTime->setTimestamp((int) $date);
        }

        return new \DateTimeImmutable((string) $date);
    }

    private static function datePart(\DateTimeInterface $date, $tipo) {
        $formats = array(
            'day' => 'd',
            'd' => 'd',
            'dd' => 'd',
        ...
        return $date->format(isset($formats[$tipo]) ? $formats[$tipo] : $tipo);
    }
