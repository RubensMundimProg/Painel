...
class AbstractEstruturaService {
    ...
    public function getTable() {
        if (!$this->table) {
            ...
            $arraySerializable = class_exists('Laminas\Hydrator\ArraySerializableHydrator')
                ? 'Laminas\Hydrator\ArraySerializableHydrator'
                : 'Laminas\Hydrator\ArraySerializableHydrator';
            $this->hydrator = (isset($this->table) && isset($this->table->table) ) ? new \Estrutura\Table\TableEntityMapper($this->table->campos) : new ArraySerializableHydrator();
            $rowObjectPrototype = $this;

            $resultSet = new \Laminas\Db\ResultSet\HydratingResultSet($this->hydrator, $rowObjectPrototype);
...
//    }
    ...
    public function getAdapter() {
        ...
        if (!self::$adapter) {            
            ...
            self::$adapter = new \Laminas\Db\Adapter\Adapter($this->getConfig());
        }
        return self::$adapter;
    }
