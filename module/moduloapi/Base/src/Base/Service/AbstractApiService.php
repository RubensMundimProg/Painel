...
class AbstractApiService {
    ...
    public function hydrate($attribute = null, $clear = true) {
        ...
        if ($attribute) {
            ...
            if ($this->table && isset($this->table->campos)) {
        ...
        if(property_exists($this, 'customAttributes') && $this->customAttributes != false && count($this->customAttributes)){
    ...
    private function setHydrator()
    {
        ...
        $this->hydrator = (isset($this->table) && isset($this->table->campos))
            ? new \Estrutura\Table\TableEntityMapper($this->table->campos)
    ...
    public function epocToDate($dataRM, $formato='d/m/Y'){
        if ($dataRM === null || $dataRM === '') {
            return null;
        }

        $dataRM = (string) $dataRM;
        ...
        if (!is_numeric($data) || !is_numeric($GMT3)) {
            return null;
        }

        $GMT3 = -(int) $GMT3;
        $data = gmdate('d/m/Y H:i:s', (int) $data + 3600 * ($GMT3 + date("I")));
