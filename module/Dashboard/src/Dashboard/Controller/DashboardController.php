...
class DashboardController extends AbstractEstruturaController
{
    ...
    public function getDashboardJsonAction()
    {
        ...
        if ($request->isPost()) {
            ...
            if (count($post)) {
                ...
                $sistema = $container->offsetGet('sistema');
                $filename = "dashboard_{$sistema}";
                $file = "./data/json/{$filename}.json";
                $buildError = '';

                if (!file_exists($file) || $this->isDashboardCacheExpired($file)) {
                    try {
                        $this->buildDashboardDataAction(null, null, false);
                    } catch (\Throwable $e) {
                ...
                if (file_exists($file)) {
                    ...
                    $update = \DateTime::createFromFormat('Y-m-d H:i:s', date('Y-m-d H:i:s', $update));
                    $json = json_encode(['error' => false, 'message' => $buildError, 'dados' => file_get_contents($file), 'update' => $update->format('d/m/Y H:i:s')]);
                } else {
                    $message = $buildError ?: 'Não há massa de dados para o período e avaliação pedagógica selecionados.';
                    $json = json_encode(['error' => true, 'message' => $message, 'dados' => []]);
                }
            }
    ...
    public function buildDashboardDataAction($period = null, $filename = null, $terminate = true)
    {
        $json = [];
        if (!$terminate) {
...
//            }
    ...
    private function isDashboardCacheExpired($file)
    {
        $time = filemtime($file);
        if (!$time) {
            return true;
        ...
        return $diff->days > 0 || $diff->h > 0 || $diff->i > 30;
    }
