...
class ApiController extends AbstractEstruturaController{
    ...
    protected function inserirOcorrencia(){
       ...
       // 1. Definição da Zona de Conflito (Onde os dois exames ocorrem)
        $municipiosZonaConflito = ['PA - BELEM', 'PA - MARITUBA', 'PA - ANANINDEUA'];

        // 2. Lista de Exceção CPNU (Códigos específicos do CPNU dentro da zona de conflito)
        // Estes locais, mesmo sendo em Belém/Ananindeua, são CPNU.
        $codigosCpnuExcecao = [
            '20155', // E.E.E.F,M JOÃO CARLOS BATISTA
            '20156', // EMEF ANTONIO TEIXEIRA GUEIROS
            '20159', // EEEFM PAULINO DE BRITO
            '20158', // ROSANA BASTOS - PEDREIRA
            '20154', // EEEFM MINISTRO ALCIDES CARNEIRO
            '20157', // COLÉGIO UNIVERSO
            '20160'  // ELITE EDUCAR
        ];
        ...
        if (isset($post['coordenacao'])) {
            // Extrai apenas o número do início da coordenação para verificação (ex: "20155")
            $codigoDetectado = '';
            if (preg_match('/^(\d+)/', $post['coordenacao'], $matches)) {
                $codigoDetectado = $matches[1];
            }

            $tagAplicacao = '';

            // Verifica se estamos na Zona de Conflito
            if (in_array($municipioRecebido, $municipiosZonaConflito)) {
                
                // Estamos em Belém, Ananindeua ou Marituba.
                // Se o código for um dos listados como CPNU, recebe CPNU.
                if (in_array($codigoDetectado, $codigosCpnuExcecao)) {
                    $tagAplicacao = 'CPNU';
                } else {
                    // Se não é exceção, assume-se ENEM padrão nestas cidades
                    $tagAplicacao = 'ENEM';
                }

            } else {
                // Qualquer outro município fora da zona de conflito é CPNU
                $tagAplicacao = 'CPNU';
            }

            // Aplica a transformação final: "20155 LOCAL" -> "20155 CPNU - LOCAL"
            // Nota: O regex garante que só alteramos se começar com números seguidos de espaço
            $post['coordenacao'] = preg_replace('/^(\d+)\s+/', '$1 ' . $tagAplicacao . ' - ', $post['coordenacao']);
        }
        // --------------------------------------------------
        ...
        $service->setSistema('ENEM - CPNU');//PROVISORIAMENTE FIXADO
...
//            }
        ...
        if($filtro){
            ...
            $ocorrencia->setSistema('ENEM - CPNU');
        }
