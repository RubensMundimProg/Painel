...
{
    private function isModernUi()
    {
        $interface = new \Laminas\Session\Container('InterfacePreferencias');

        return $this->params()->fromQuery('ui') === 'modern' || $interface->offsetGet('ui') === 'modern';
    }
    ...
    public function indexAction()
    {
        ...
        $viewModel = new ViewModel(['lista' => $lista, 'service' => $service, 'form' => $form, 'sistema' => $nomeSistema, 'categoriasSelecionadas' => $categorias]);
        if ($this->isModernUi()) {
            $this->layout('layout/modern');
            $viewModel->setTemplate('application/triagem/index-modern');
        }
    ...
    public function recusadosAction()
    {
        ...
        $viewModel = new ViewModel(['lista' => $lista, 'service' => $service, 'form' => $form, 'sistema' => $nomeSistema, 'categoriasSelecionadas' => $categorias, 'recusados' => true]);
        if ($this->isModernUi()) {
            $this->layout('layout/modern');
            $viewModel->setTemplate('application/triagem/index-modern');
        }
    ...
    public function excluirTriagemAction()
    {
        ...
        $service = new \Classes\Service\Alertas();
        $container = new \Laminas\Session\Container('UsuarioApi');
    ...
    public function confirmarTriagemAction()
    {
        try {
            ...
            $container = new \Laminas\Session\Container('SistemaSelecionado');
...
//                        'informacoes_adicionais'=>$alerta->getInformacoesAdicionais(),
                ...
                */
                ...
                $this->addAlertAttachments($event, $alerta->getAnexo());
                ...
                $service->salvar();
                $this->syncValidadosCacheAfterTriagem($event, $alerta, $evento, $statusAlerta);
        ...
        try {
            ...
            if ($this->params('id')) { //GET
                ...
                if ($request->isPost()) {
                    ...
                    if ($this->hasUploadedPaths($uplod)) {
...
//                }
        ...
        $viewModel = new ViewModel(['form' => $form, 'sistema' => $nomeSistema]);
        if ($this->isModernUi()) {
            $this->layout('layout/modern');
            $viewModel->setTemplate('application/triagem/cadastro-ocorrencia-modern');
        }
...
//        }
    ...
    {
        $anexo = base64_decode((string)$this->params('id'), true);
        $absoluto = $anexo ? $this->resolvePublicAttachmentPath($anexo) : null;
        ...
        $filename = basename($absoluto);
        $response = $this->getResponse();
        $headers = $response->getHeaders();
        $headers->addHeaderLine('Content-Type', 'application/octet-stream');
        $headers->addHeaderLine('Content-Transfer-Encoding', 'Binary');
        $headers->addHeaderLine('Content-Disposition', 'attachment; filename="' . addslashes($filename) . '"');
        $headers->addHeaderLine('Content-Length', (string)filesize($absoluto));
        $response->setContent(file_get_contents($absoluto));
...
//                    'caminho_critico'=>'2. Implementação - Aplicação do exame', //Obsoleto
    ...
    private function hasUploadedPaths(array $upload)
    {
        foreach ($upload as $paths) {
            if (trim((string)$paths) !== '') {
                return true;
            }
        }
    ...
    private function addAlertAttachments(Event $event, $anexos)
    {
        if (!$anexos) {
            return;
        }

        foreach (explode('|', (string)$anexos) as $file) {
            $file = trim($file);
            if ($file === '') {
                continue;
            }

            $anexo = $this->resolvePublicAttachmentPath($file);
            if (!$anexo || !is_readable($anexo)) {
                $this->logTriagemWarning('Anexo nao encontrado: ' . $file);
                continue;
            }

            $event->addAttachment(basename($anexo), 'anexar_arquivos', file_get_contents($anexo));
        }
    }

    private function resolvePublicAttachmentPath($file)
    {
        ...
        $publicRoot = realpath(BASE_PATCH . DIRECTORY_SEPARATOR . 'public');
        if (!$publicRoot) {
            return null;
        }
        ...
        if (preg_match('/^[A-Za-z]:\//', $file)) {
            $candidate = $file;
        } elseif (str_starts_with($file, './public/') || str_starts_with($file, 'public/')) {
            $candidate = BASE_PATCH . DIRECTORY_SEPARATOR . preg_replace('#^\./#', '', $file);
        } elseif (str_starts_with($file, '/')) {
            $candidate = $publicRoot . str_replace('/', DIRECTORY_SEPARATOR, $file);
        } else {
            $candidate = $publicRoot . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $file);
        }
        ...
        $root = rtrim($publicRoot, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR;
        if (stripos($realPath . DIRECTORY_SEPARATOR, $root) !== 0) {
            return null;
        }
    ...
    private function logTriagemWarning($message)
    {
        file_put_contents('./data/erro_triagem.txt', '[' . date('Y-m-d H:i:s') . '] ' . $message . PHP_EOL, FILE_APPEND);
    }

    private function syncValidadosCacheAfterTriagem(Event $event, Alertas $alerta, array $evento, $statusAlerta)
    {
        $eventId = $event->getCode();
        if (!$eventId) {
            $this->logTriagemWarning('Cache dos validados nao atualizado: evento sem codigo RM.');
            return;
        }

        $status = ((int)$statusAlerta === 2) ? 'Fechado' : 'Aberto';
        $now = $this->dateToRiskManagerJsonDate();
        $customAttributes = $evento['customAttributes'];
        $customAttributes = array_merge([
            'Assets' => $alerta->getMunicipio(),
            'Coordinator' => null,
            'Created' => $now,
            'Deadline' => null,
            'Description' => $evento['description'],
            'EndDate' => $status === 'Fechado' ? $now : null,
            'EventID' => $eventId,
            'ExpectedEndDate' => null,
            'Involveds' => null,
            'LastProgressComment' => null,
            'Latitude' => null,
            'Longitude' => null,
            'Responsible' => null,
            'StartDate' => null,
        ...
        $customAttributes['EventID'] = $eventId;
        $customAttributes['Status'] = $status;
        $customAttributes['Title'] = $evento['title'];
        $customAttributes['Description'] = $evento['description'];
        $customAttributes['Type'] = $evento['eventType'];
        $customAttributes['Created'] = $customAttributes['Created'] ?: $now;
        $customAttributes['UpdatedOn'] = $now;

        $anexos = [];
        if ($alerta->getAnexo()) {
            foreach (explode('|', (string)$alerta->getAnexo()) as $anexo) {
                $anexo = trim($anexo);
                if ($anexo !== '') {
                    $anexos[] = $anexo;
                }
        ...
        $cacheItem = [
            ...
            'code' => $eventId,
            'created' => $customAttributes['Created'],
            'updatedOn' => $now,
        ...
        foreach (['./data/cache/validados.json', './public/filter/cache_alertas.json'] as $cacheFile) {
            $this->upsertValidadosCacheItem($cacheFile, $cacheItem, $eventId);
        }

        file_put_contents('./data/validados_history.txt', 'Atualizado pela triagem as: ' . date('d/m/Y H:i:s'));
    }

    private function upsertValidadosCacheItem($cacheFile, array $cacheItem, $eventId)
    {
        ...
        foreach ($cache as $key => $item) {
            $itemId = $item['customAttributes']['EventID'] ?? $item['code'] ?? null;
            if ((string)$itemId === (string)$eventId) {
                $cache[$key] = $cacheItem;
                $updated = true;
                break;
            }
        ...
        if (!$updated) {
            array_unshift($cache, $cacheItem);
        }

        file_put_contents($cacheFile, json_encode($cache, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));
    }

    private function dateToRiskManagerJsonDate()
    {
        return '/Date(' . (int)round(microtime(true) * 1000) . date('O') . ')/';
    }
