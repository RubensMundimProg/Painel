        ...
        try{
            ...
            if($request->isPost() && isset($post['uf']) && isset($post['dataInicio']) && isset($post['dataFim'])) {
                ...
                foreach ($data as $item) {
                    ...
                    if(empty($item['customAttributes']['categoria_evento']) || empty($item['customAttributes']['municipio_de_aplicacao'])) continue;
                   ...
                   // $dtCreated = '2019-11-30 17:30';
                    ...
                    if (($created >= $diaInicioSelecionado) && ($created <= $diaFimSelecionado)){
                        ...
                        $municipio = isset($local[1]) ? trim($local[1]) : '';
...
//                        if($categoriaSelecionada && ($categoriaSelecionada != $item['customAttributes']['categoria_evento'])) continue; //old one
                        ...
                        if($categoria[0] == 'Segurança Pública'){
                            $subCategoria = isset($categoria[1]) ? $categoria[1] : '';
                            @$tabelaSegurancaPublica[$subCategoria]++;
                        }
                        ...
                        if (!isset($item['anexo']) || !is_array($item['anexo'])) {
                            $item['anexo'] = [];
                        }
                ...
                $pdfGenerator = $this->getServiceLocator()->get(PdfGenerator::class);
                $pdfContent = $pdfGenerator->renderTemplate(
                    'application/relatorios/gerar',
                    $pdfJsonArray,
                    array(
                        'paperSize' => 'A4',
                        'paperOrientation' => 'landscape',
                        'basePath' => BASE_PATCH . DIRECTORY_SEPARATOR . 'public',
                    )
                );
                ...
                $headers->addHeaderLine('Content-Disposition', 'inline; filename="' . str_replace('"', '', $fileName) . '"');
