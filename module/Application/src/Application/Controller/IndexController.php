...
class IndexController extends AbstractEstruturaController
{
    ...
    public function indexAction()
    {
        ...
        if ($interface->offsetGet('ui') === 'modern') {
            $viewModel->setTemplate('application/index/index-modern');
        }
    ...
    public function meuEspacoAction()
    {
        ...
        $viewModel = new ViewModel(['nome' => $usuarioApi->get('nome'), 'perfis' => $perfis, 'email' => $usuarioApi->get('email'), 'lista' => $lista]);
        $interface = new Container('InterfacePreferencias');
        if ($interface->offsetGet('ui') === 'modern') {
            $viewModel->setTemplate('application/index/meu-espaco-modern');
        }
