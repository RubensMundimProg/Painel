...
$(function(){
        ...
        $.ajax({
            ...
            success:function(res){
                ...
                if(!res.error){
                    ...
                    if(tipo == 'Fechar'){
                        activeRow.find('.status-registro').html(res.status);
                        if(res.removeFromList){
                            activeRow.fadeOut(250, function(){
                                $(this).remove();
                            });
                        }
                    }
                ...
                if(!(tipo == 'Fechar' && res.removeFromList)){
    ...
    $('body').on('click','.btn-confirma', function(){
        ...
        $('.hidden-print a, .hidden-print button').addClass('hide');
        ...
        $.ajax({
            ...
            success:function(json){
                $('.hidden-print a, .hidden-print button').removeClass('hide');
    ...
    $('body').on('click', '.view-progress', function(){
        $(this).parents("tr").addClass('active');
...
function showMsg(msg,toggle,error){
    ...
    var closeButton = '<button type="button" class="btn-close close" data-bs-dismiss="alert" data-dismiss="alert" aria-label="Fechar"><span aria-hidden="true">&times;</span></button>';
...
//    showMsg(1° parameter->the message itself OR return object['error':'','message':''], 2° parameter->true uses array from the first parameter AND false uses string of message, 3° parameter-> true for error type error AND false for error type success, apply only when 1°parameter is a string of message);
    if(toggle){
        ...
        $('body').append('<div class="container-alertas"><div class="alert '+type_alert+' alert-dismissible fade show" role="alert">'+closeButton+'<ul><li class="error">'+msg['message']+'</li></ul></div></div>');
    }else{
        var type_alert = (error)?'alert-danger':'alert-success';
        $('body').append('<div class="container-alertas"><div class="alert '+type_alert+' alert-dismissible fade show" role="alert">'+closeButton+'<ul><li class="error">'+msg+'</li></ul></div></div>');
    }
